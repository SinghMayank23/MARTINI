...
public:
  ...
  bool init2( double jetpTmin, double jetpTmax, bool doMPIinit, int iDiffSysIn, Info* infoPtrIn,
    Settings& settings, ParticleData* particleDataPtr, Rndm* rndmPtrIn,
    BeamParticle* beamAPtrIn, BeamParticle* beamBPtrIn,
    Couplings* couplingsPtrIn, PartonSystems* partonSystemsPtrIn,
    SigmaTotal* sigmaTotPtrIn, UserHooks* userHooksPtrIn);
  ...
  //added by Bjoern Schenke 27/08/2009 to pass cross sections to MARTINI
  double returnJetCrossSection() const {return jetXSec;}
  double returnTotalJetCrossSection() const {return totaljetXSec;}
  double returnTotalCrossSection() const {return totalXSec;}
  double returnElasticCrossSection() const {return elasticXSec;}
...
private:
  ...
  int    pTmaxMatch, alphaSorder, alphaEMorder, alphaSnfmax, bProfile,
         ...
         enhanceScreening;
  double alphaSvalue, Kfactor, pT0Ref, ecmRef, ecmPow, pTmin, mypTmin, coreRadius,
  ...
  double eCM, sCM, pT0, pT20, pT2min, mypT2min, pTmax, mypTmax,
         pT2max, mypT2max, pT20R, pT20minR, mypT20minR,pT20maxR, mypT20maxR, 
         pT20min0maxR, mypT20min0maxR, pT2maxmin, mypT2maxmin, sigmaND,
         pT4dSigmaMax, pT4dProbMax, dSigmaApprox, sigmaInt, sudExpPT[101],
         zeroIntCorr, normOverlap, nAvg, kNow, normPi, bAvg, bDiv,
         probLowB, radius2B, radius2C, fracA, fracB, fracC, fracAhigh,
         fracBhigh, fracChigh, fracABChigh, expRev, cDiv, cMax, jetXSec, 
         totaljetXSec, totalXSec, elasticXSec;
  ...
  //added by Bjoern Schenke 27/08/2009 to pass cross sections to MARTINI
  void jetCrossSection2( Settings& settings);
  ...
  //added by Bjoern Schenke 27/08/2009 to pass cross sections to MARTINI
  double mysigmaPT2scatter( Settings& settings, bool isFirst = false);
