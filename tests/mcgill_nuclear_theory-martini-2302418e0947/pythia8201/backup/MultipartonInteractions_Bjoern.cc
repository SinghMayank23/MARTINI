...
bool MultipartonInteractions::init2( double jetpTmin, bool doMPIinit, int iDiffSysIn,
  Info* infoPtrIn, Settings& settings, ParticleData* particleDataPtr,
  Rndm* rndmPtrIn, BeamParticle* beamAPtrIn, BeamParticle* beamBPtrIn,
  Couplings* couplingsPtrIn, PartonSystems* partonSystemsPtrIn,
  ...
  for (int iStep = 0; iStep < nStep; ++iStep) {
    ...
    for ( ; ; ) {
      ...
      mypT20maxR     = pT2max;
      pT20min0maxR   = pT20minR * pT20maxR;
      mypT20min0maxR = mypT20minR * mypT20maxR;
      pT2maxmin      = pT2max - pT2min;
      mypT2maxmin    = pT2max - mypT2min; 

      // Provide upper estimate of interaction rate d(Prob)/d(pT2).
      upperEnvelope2( settings );
...
void MultipartonInteractions::upperEnvelope2( Settings& settings ) {
...
void MultipartonInteractions::jetCrossSection2( Settings& settings ) {
  ...
  // Common factor from bin size in dpT2 / (pT2 + r * pT20)^2 and statistics.
  double sigmaFactor = (1. / mypT20minR - 1. / mypT20maxR) / (100. * nSample);
  ...
  for (int iPT = 99; iPT >= 0; --iPT) {
    ...
    for (int iSample = 0; iSample < nSample; ++iSample) {
      double mappedPT2 = 1. - 0.01 * (iPT + rndmPtr->flat());
      pT2 = mypT20min0maxR / (mypT20minR + mappedPT2 * mypT2maxmin) - 0.;
...
double MultipartonInteractions::mysigmaPT2scatter( Settings& settings, bool isFirst) {
  ...
  // double pT2massive = dSigmaDtSel->pT2MPI();
  double pT2massive = dSigmaDtSel->pT2MPI();
  dSigmaCorr *= pow2( pT2massive / (pT2massive + mypT20) );
