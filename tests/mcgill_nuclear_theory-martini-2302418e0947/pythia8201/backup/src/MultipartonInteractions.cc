...
bool MultipartonInteractions::init2( double jetpTmin, double jetpTmax, bool doMPIinit, int iDiffSysIn,
  Info* infoPtrIn, Settings& settings, ParticleData* particleDataPtr,
  Rndm* rndmPtrIn, BeamParticle* beamAPtrIn, BeamParticle* beamBPtrIn,
  Couplings* couplingsPtrIn, PartonSystems* partonSystemsPtrIn,
  ...
  for (int iStep = 0; iStep < nStep; ++iStep) {
    ...
    if (jetpTmin == 0) 
    mypTmin        = pTmin;
    else mypTmin   = jetpTmin;
    pT20           = pT0*pT0;
    mypT2min       = mypTmin*mypTmin;
    if (jetpTmax > jetpTmin)
    mypTmax        = jetpTmax;
    else mypTmax   = 0.5*eCM;
    mypT2max         = mypTmax*mypTmax;
    pT20R          = RPT20 * pT20;
    mypT20minR     = mypT2min + pT20R;
    pT20maxR       = mypT2max + pT20R;
    mypT20min0maxR = mypT20minR * pT20maxR;
    mypT2maxmin    = mypT2max - mypT2min; 

    cout << "computing jet cross section for MARTINI ..." << endl;
    jetCrossSection2( settings );

    jetXSec = sigmaInt; // set the jet cross section to be used by MARTINI
    totalXSec = sigmaTotPtr->sigmaTot();
    elasticXSec = sigmaTotPtr->sigmaEl();
...
//      double mappedPT2 = 1. - 0.01 * (iPT + rndmPtr->flat());
      double mappedPT2 = 1. - 0.01 * (iPT + 0.5);
...
// added by Bjoern Schenke 08/27/2009 to compute jet cross section 
// given a certain minimal p_T cutoff and pass it to MARTINI
void MultipartonInteractions::jetCrossSection2( Settings& settings ) {

  // Common factor from bin size in dpT2 / (pT2 + r * pT20)^2 and statistics.
  double sigmaFactor = (1. / mypT20minR - 1. / pT20maxR) / (100. * nSample);
...
//      double mappedPT2 = 1. - 0.01 * (iPT + rndmPtr->flat());
      double mappedPT2 = 1. - 0.01 * (iPT + 0.5);
      pT2 = mypT20min0maxR / (mypT20minR + mappedPT2 * mypT2maxmin) - pT20R;

      // Evaluate cross section dSigma/dpT2 in phase space point.
      double dSigma = mysigmaPT2scatter( settings, true);
...
cout << "[sig] " << scientific << dSigmaPartonCorr << " " << volumePhSp << endl;
...
// added by Bjoern Schenke 08/27/2009 to compute jet cross section 
// given a certain minimal p_T cutoff and pass it to MARTINI
double MultipartonInteractions::mysigmaPT2scatter( Settings& settings, bool isFirst) {
 
  double renMultFac = settings.parm("SigmaProcess:renormMultFac"); 
  double facMultFac = settings.parm("SigmaProcess:factorMultFac"); 
  // Derive recormalization and factorization scales, amd alpha_strong/em.
  pT2shift = pT2 + pT20;
  pT2Ren   = pT2shift*renMultFac;
  pT2Fac   = (SHIFTFACSCALE) ? pT2shift : pT2;
  pT2Fac *= facMultFac;
  alpS  = alphaS.alphaS(pT2Ren);
  alpEM = alphaEM.alphaEM(pT2Ren);
  ...
  double volumePhSp = pow2(2. * yMax);
  double dSigmaScat = dSigmaPartonCorr * xPDF1sum * xPDF2sum * volumePhSp;
cout << "[mysig] " << scientific << dSigmaPartonCorr << " " << volumePhSp << endl;
  // Dampen cross section at small pT values; part of formalism.
  // Use pT2 corrected for massive kinematics at this step.??
  // double pT2massive = dSigmaDtSel->pT2MPI();
  double pT2massive = dSigmaDtSel->pT2MPI();
