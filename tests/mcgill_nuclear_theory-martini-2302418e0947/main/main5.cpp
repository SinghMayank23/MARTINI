...
int main(int argc, char* argv[]) 
{
  ...
  string n_string = "pp_partons_statuschecked_lowstats_differentbinning";
  ...
  while(event_counter<numJets)      // The main loop
	...
	Vec4 vecp = plist[0]->at(ip).p();
	int statusp = plist[0]->at(ip).status();
	double etap = 0.5*log(sqrt( vecp.px()*vecp.px()+vecp.py()*vecp.py() )/vecp.pz());
	if(fabs(etap) < yMax && statusp > 0){ 
	  double Ep = vecp.e();
	  double pxp = vecp.px();
	  double pyp = vecp.py();
	  double pzp = vecp.pz();
	...
	if(jets[ij].mperp() > ET_gt){
	  ...
	  yLeadingJet = jets[i_greatest_ET].pseudorapidity();
	}
      }
	...
	if(jets[ij].mperp() > ET_lt
      ...
      if(ET_gt > ETgt_min && ET_lt > ETlt_min && fabs(yLeadingJet) < yLeadingJetMax){
	
	//Output the particles in this event:
	for(int ip=0; ip<fullEventSize; ip++){
	  Vec4 vecp = plist[0]->at(ip).p();
	  int statusp = plist[0]->at(ip).status();
	  double etap = 0.5*log(sqrt( vecp.px()*vecp.px()+vecp.py()*vecp.py() )/vecp.pz());
	  if(fabs(etap) < yMax && statusp > 0){ 
	    ...
	    double pyp = vecp.py();
	    double pzp = vecp.pz();
	    foutEvents << Ep << " " << pxp << " " << pyp << " " << pzp << endl;
	  }
	}
