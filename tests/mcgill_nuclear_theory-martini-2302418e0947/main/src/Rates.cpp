...
Norms Rates::integrate(double p, double T, double alpha_s, int Nf, int Ldependence, double L, bool runningCoupling)
{
  Norms norms;
  double L_fac;
  ...
  if(runningCoupling == 1) //Multiply the rates by a factor taking into account the running of \alpha_s:
    {
      double runningFactor = log(sqrt(4.*M_PI*alpha_s)*T*pow(10./T, 0.25)/.175)
	/log(sqrt(4.*M_PI*alpha_s)*T*pow(p/T, 0.25)/.175);
      if(runningFactor < 1.){
	norms.Gamma *= runningFactor;
	norms.Gamma_ggg *= runningFactor;
	norms.Gamma_gqq *= runningFactor;
	norms.Gamma_em *= runningFactor;
      }
    }
