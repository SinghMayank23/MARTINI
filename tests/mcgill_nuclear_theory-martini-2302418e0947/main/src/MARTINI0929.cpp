		  ...
		  if ( random->genrand64_real1() < totalHQXSec/inelasticXSec )
		    {
		      ...
		      for (int ip = 0; ip < pythia.event.size(); ++ip) 
			{
			  //cout << "Was this loop entered?" << endl;
			  //cout << ip << " " << pythia.event[ip].status() << " " << pythia.event[ip].id() << endl;
			  if (pythia.event[ip].status()>0 && (abs(pythia.event[ip].id())==4 || abs(pythia.event[ip].id()) == 5) )
			    ...
			    {
			      //cout << "Was this if statement entered?" << endl;
			      parton.id(pythia.event[ip].id());                     // set parton id
			      parton.status(pythia.event[ip].status());             // set parton status
			      parton.mass(pythia.event[ip].m());                // set mass
			      if (fixedTemperature==0)
				{
				  ...
				  if(abs(parton.id()) == 4){
				    ...
				    //dz = gsl_ran_gaussian(_gsl_rand, charmWidth);
				  }
				  if(abs(parton.id()) == 5){
				    dx = gsl_ran_gaussian(gsl_rand, bottomWidth);
				    dy = gsl_ran_gaussian(gsl_rand, bottomWidth);
				    //dz = gsl_ran_gaussian(gsl_rand, bottomWidth);
				  }
			      ...
			      else
				{
				  ...
				  if(abs(parton.id()) == 4){
				    dx = gsl_ran_gaussian(gsl_rand, charmWidth);
				    dy = gsl_ran_gaussian(gsl_rand, charmWidth);
				    //dz = gsl_ran_gaussian(gsl_rand, charmWidth);
				  }
				  if(abs(parton.id()) == 5){
				    dx = gsl_ran_gaussian(gsl_rand, bottomWidth);
				    dy = gsl_ran_gaussian(gsl_rand, bottomWidth);
      ...
      for (int i = 0; i < pythia.event.size(); ++i) 
	{
	  if (pythia.event[i].isFinal() && (abs(pythia.event[i].id()) == 4 || abs(pythia.event[i].id()) == 5) )
		...
		parton.tini(0.);
	      ...
	      /*
		cout << "What's up, is this getting called, why is this here?" << endl;
		if (evolution && fixedTemperature==0)
		{
		//parton.x(0.);                                   // set position
		//parton.y(0.);
...
int MARTINI::evolve_HQ(vector<Parton>  **plist, int counter, int it)
{
  ...
  for ( int i=0; i<imax; i++)                                   // loop over all partons
	...
	if (fixedTemperature == 0)
	  {
		...
		xnew = x+vecp.px()/sqrt(vecp.pAbs()*vecp.pAbs()+M*M)*dtfm;      // update x position for a massless parton (vel=c)
		ynew = y+vecp.py()/sqrt(vecp.pAbs()*vecp.pAbs()+M*M)*dtfm;      // update y position 
		znew = z+vecp.pz()/sqrt(vecp.pAbs()*vecp.pAbs()+M*M)*dtfm;      // update z position 
		//cout << "pz = " << vecp.pz() << ", dtfm = " << dtfm << ", znew = " << znew << ", t = " << t << endl;
		
		if(z*z<t*t) tau = sqrt(t*t-z*z);
		//ONLY move the quarks if tau<hydroTau0 here -- if tau>hydroTau0 then the 
		//spatial evolution is determined in the examineHQ switchings:
		if(tau<hydroTau0){
		  plist[0]->at(i).x(xnew);
		  plist[0]->at(i).y(ynew);
		  plist[0]->at(i).z(znew);
	    ...
	    umu[2] = gamma*vy;
	    umu[3] = gamma*vz;
	    //cout << "umu: " << umu[0] << " " << umu[1] << " " << umu[2] << " " << umu[3] << endl;
	    //cout << "pmu: " << pmu[0] << " " << pmu[1] << " " << pmu[2] << " " << pmu[3] << endl;
	    LorentzBooster(umu, pmu, pmuRest);                      // Boost into the fluid's rest frame
	    //cout << "pmu: " << pmuRest[0] << " " << pmuRest[1] << " " << pmuRest[2] << " " << pmuRest[3] << endl;
	...
	if(examineHQ == 1){
	  ...
	  //cout << "We have gotten this far..." << endl;
	  //cout << T_C_HQ << endl;
	  //cout << "T = " << T << endl;
	  if(T > T_C_HQ){
	    plist[0]->at(i).tFinal(t); // update the time - finally will be the final time
	    ...
	    //cout << "pmuNew: " << pmuNew[0] << " " << pmuNew[1] << " " << pmuNew[2] << " " << pmuNew[3] << endl;
	    
	    //Update the particle's momentum:
	    plist[0]->at(i).p(pmuNew[1], pmuNew[2], pmuNew[3]);
	    //cout << "pmuNew: " << pmuNew[1] << " " << pmuNew[2] << " " << pmuNew[3] << endl;
	  }
	...
	if(examineHQ == 2){
	  //cout << "Is this ever entered?" << endl;
	  if(id == 4 || id == 5){ //Only the quarks, not the anti-quarks!
	  //cout << "How about this, is 4729 reached?" << endl;
	    antiI = plist[0]->at(i).antiI();
	    //cout << "plist[0]->size() = " << plist[0]->size() << endl;
...
int MARTINI::fragmentation_HQ(vector<Parton> ** plist){
  ...
  if(fullEvent == 0 || totalNNs == 1){
    ...
    //double A = (p1mu[1]-p2mu[1])*(p1mu[1]-p2mu[1])+(p1mu[2]-p2mu[2])*(p1mu[2]-p2mu[2])
    //          +(p1mu[3]-p2mu[3])*(p1mu[3]-p2mu[3])-(p1mu[0]-p2mu[0])*(p1mu[0]-p2mu[0]);
    //double prel2 = (A*A+2.*A*(M1*M1+M2*M2)+(M1*M1-M2*M2)*(M1*M1-M2*M2))
    //	       /(A+2.*(M1*M1+M2*M2) );
    ////In the end, set prel2 equal to the simpler, equal-mass result:
    //prel2 = -(p1mu[0]-p2mu[0])*(p1mu[0]-p2mu[0])+(p1mu[1]-p2mu[1])*(p1mu[1]-p2mu[1])
    //  +(p1mu[2]-p2mu[2])*(p1mu[2]-p2mu[2])+(p1mu[3]-p2mu[3])*(p1mu[3]-p2mu[3]);
    //The reduced mass:
    //double mred = M1*M2/(M1+M2);
    //Finally, the non-relativistic energy of the pair in the CM frame:
  ...
  if(fullEvent==1 && totalNNs > 1 ){
	...
	M2 = BOTTOM_MASS;
      ...
      if(tau<hydroTau0) notstatistical[ipair] = 1;
    }
	...
	if(E_CM > E_CCB_BOUND){
	  //double pt1before = sqrt(p1mu[1]*p1mu[1]+p1mu[2]*p1mu[2]);
	  //double y1before = 0.5*log((sqrt(M1*M1+pt1before*pt1before+p1mu[3]*p1mu[3])+p1mu[3])/(sqrt(M1*M1+pt1before*pt1before+p1mu[3]*p1mu[3])-p1mu[3]));
	  //double pt2before = sqrt(p2mu[1]*p2mu[1]+p2mu[2]*p2mu[2]);
	  //double y2before = 0.5*log((sqrt(M2*M2+pt2before*pt2before+p2mu[3]*p2mu[3])+p2mu[3])/(sqrt(M2*M2+pt2before*pt2before+p2mu[3]*p2mu[3])-p2mu[3]) );
	  //cout << "y1before = " << y1before << ", y2before = " << y2before << endl;
	  ...
	  //double pt1after = sqrt(pnew1[1]*pnew1[1]+pnew1[2]*pnew1[2]);
	  //double y1after = 0.5*log((sqrt(DMASS*DMASS+pt1after*pt1after+pnew1[3]*pnew1[3])+pnew1[3])/(sqrt(DMASS*DMASS+pt1after*pt1after+pnew1[3]*pnew1[3])-pnew1[3]));
	  //double pt2after = sqrt(pnew2[1]*pnew2[1]+pnew2[2]*pnew2[2]);
	  //double y2after = 0.5*log((sqrt(DMASS*DMASS+pt2after*pt2after+pnew2[3]*pnew2[3])+pnew2[3])/(sqrt(DMASS*DMASS+pt2after*pt2after+pnew2[3]*pnew2[3])-pnew2[3]));
	  //cout << "DMASS: " << DMASS << endl;
	  //cout << "pnew1: " << pnew1[1] << " " << pnew1[2] << " " << pnew1[3] << endl;
	  //cout << "pnew2: " << pnew2[1] << " " << pnew2[2] << " " << pnew2[3] << endl;
	  //cout << "y1after = " << y1after << ", y2after = " << y2after << endl;
