...
int MARTINI::evolve(vector<Parton>  **plist, int counter, int it)
{
  ...
  for ( int i=0; i<imax; i++)                                   // loop over all partons
  	...
  	if (tauEtaCoordinates == 1) 
			{
				eta = 0.;
				if (t == 0. && z == 0.) eta = 0.;
			  else if ( t > 0 && fabs(z)<t ) eta = 0.5 * log ( (t+z)/(t-z) );							//z is converted to eta in tauetacoordinates
				else {
					if(!trackHistory) {
	...
	}																												// hydroZmax, hydroDz are actually hyDroEtamax, hydroDeta in tauetacoordinates
      ...
      if (fixedTemperature == 0)
	{
	  ...
	  if(tauEtaCoordinates==1){
	    hydroInfo = hydroSetup->getHydroValuesTauEta(x, y, z, t, hydroXmax, hydroZmax, hydroTauMax, hydroTau0, 
	  ...
	  if(pRest < pCut) continue; //New use for pCut
...
// see if parton i has a mother or a daughter
      in_coherence = 0; // default no
  		if (rateSelector == 8) {
      	if(plist[0]->at(i).daughter() != -1) in_coherence = hasDaughter(plist[0], i);
		   	if(plist[0]->at(i).mother() != -1) in_coherence = hasMother(plist[0], i);
			}
...
// Sangyong's addition ends
      ...
      if (doRadiative == 1 && pRest/T > AMYpCut) 
	{
	  n = rates->integrate(pRest/T,T,alpha_s, Nf, Ldependence, lengthTraveled, runningCoupling);          // n stores the areas under the prob. distr.
      ...
      if ( abs(id) > 0 && abs(id) < 4 ) // if parton is a quark (u, d, or s), let it evolve like a quark
	{
	  ...
	  if (doRadiative == 1 && doElastic == 1 ) //radiative + elastic 
	    {
	      ...
	      if ( random->genrand64_real1() < totalQuarkProb ) // check if something happens with the quark
		{
		 ...
		 if(pRest/T>AMYpCut){
		  ...
		  if( AccProb <= randx && randx < (AccProb + NextProb) ) // qq scattering
		   {
		    elastic_qq = 1;
		   }
		  ...
		  if( AccProb <= randx && randx < (AccProb + NextProb) ) // qg scattering
		   {
		    elastic_qg = 1;
		   }
		  ...
		  if(AccProb <= randx && randx < (AccProb + NextProb)) // radiation 
		   {
		    if(in_coherence==0) radiate = 1;
		   }
		  
		  // +dt/gamma*conversionqg
		  AccProb += NextProb;
		  NextProb = (dt/gamma*conversionqg)/totalQuarkProb; 
		  if( AccProb <= randx && randx < (AccProb + NextProb) ) // qg conversion
		   {
		    if(in_coherence==0) convert_quark = 1;
		   }
		  
		  // +dt/gamma*conversionqgamma; 
		  AccProb += NextProb;
		  NextProb = (dt/gamma*conversionqgamma)/totalQuarkProb; 
		  if( AccProb <= randx && randx < (AccProb + NextProb) ) // qgamma conversion
		   {
		    if(in_coherence==0) convert_quark_to_gamma = 1;
		   }
		  ...
		  AccProb += NextProb;
		  NextProb = EMProb;
		  if( AccProb <= randx && randx <= (AccProb + NextProb) ) // PhotonRadiation
		   {
		    radiatePhoton = 1;
		   }
		  }// if(pRest/T>AMYpCut)
	  ...
	  //Here ends the modification of the rates for quarks, on to gluons. -CFY
	  else if (doRadiative == 1 && doElastic == 0 && pRest/T>AMYpCut) //radiative only
	    {
		  // Sangyong's addition:
	      ...
	      else 
		{
		  ...
		  continue;
		}// nothing happens to the quark
	    } // rad only
	  else if (doRadiative == 0 && doElastic == 1) //elastic only
	  ...
	  if( radiate == 1 ) // see if emission happens
	      ...
	      if(pRest/T>AMYpCut) // do not evolve partons with momenta below this scale 
		{
		  ...
		  if (f.y*T>pCut)                      // if new parton is kept (f.y*T > threshold [in GeV])
		      ...
		      else if (acol!=0)                         // if we had an anti-quark
			{
		      ...
		      daughter = plist[0]->size(); // the position of daughter in the plist when push_back'ed
		      newOne.plist_pos(daughter);
		      newOne.daughter_of(i); // daughter of the current parton
		      newOne.mother_of(-1);  // mother of nobody
		      newOne.elastic_only(1); // in coherence with mother
		      newOne.p_at_split(newOne.p()); // momentum at the split point

		      plist[0]->at(i).daughter_of(-1); // daughter of nobody
		      plist[0]->at(i).mother_of(daughter);
		      plist[0]->at(i).elastic_only(1); // in coherence with daughter
		      plist[0]->at(i).p_at_split(plist[0]->at(i).p());
		      //Sangyong's addition
	      ...
	      else if (acol!=0)
		{
	      ...
	      //Sangyong's addition for thermal parton from conversion process
	      newOne.plist_pos(plist[0]->size());
	      newOne.daughter_of(-1); // daughter of nobody
	      newOne.mother_of(-1);  // mother of nobody
	      newOne.elastic_only(0); // allow all scatterings
	      newOne.p_at_split(newOne.p()); // momentum at the conversion point

	      plist[0]->at(i).daughter_of(-1); // daughter of nobody
	      plist[0]->at(i).mother_of(-1); // mother of nobody
	      plist[0]->at(i).elastic_only(0); // allow all scatterings
	      plist[0]->at(i).p_at_split(plist[0]->at(i).p()); // mometum at the conversion point
	      //Sangyong's addition
      ...
      if ( id == 21 ) // if parton is a gluon, let it evolve like a gluon
	{
	  ...
	  if (doRadiative == 1 && doElastic == 1 )  //radiative + elastic
		...
		totalGluonProb = dt/gamma*(gqRate)+dt/gamma*(ggRate)+dt/gamma*conversiongq; 
	      }
	      ...
	      if ( random->genrand64_real1() < totalGluonProb ) // check if something happens with the quark
		{
		  ...
		  if( AccProb <= randx && randx < (AccProb + NextProb) ) // gq scattering
		   {
		    elastic_gq = 1;
		   }
		  ...
		  if( AccProb <= randx && randx < (AccProb + NextProb) ) // gg scattering
		   {
		    elastic_gg = 1;
		   }
		  ...
		  if(AccProb <= randx && randx < (AccProb + NextProb)) // radiate ggg
		   {
		    if(in_coherence==0) radiate_ggg = 1;
		   }
		  
		  // dt/gamma*(n.Gamma_gqq)
		  AccProb += NextProb;
		  NextProb = dt/gamma*(n.Gamma_gqq)/totalGluonProb;
		  if(AccProb <= randx && randx < (AccProb + NextProb)) // radiate gqq
		   {
		    if(in_coherence==0) radiate_gqq = 1;
		   }
		  ...
		  if( AccProb <= randx && randx < (AccProb + NextProb) ) // qg conversion
		   {
		    if(in_coherence==0) convert_gluon = 1;
		   }
	  ...
	  else if (doRadiative == 0 && doElastic == 1 ) //elastic
	  // no sense implementing coherence here
	    {
		  ...
		  if ( random->genrand64_real1() < jetXSec/inelasticXSec )
		    {
		      ...
		      if ( random->genrand64_real1() > Z/A ) n1=2112;
		      else n1=2212;
		      if ( random->genrand64_real1() > Z/A ) n2=2112;
		      else n2=2212;
		      ...
		      for (int ip = 0; ip < pythia.event.size(); ++ip) 
			{
			  if (pythia.event[ip].isFinal())// && (pythia.event[i].id()<4 || pythia.event[i].id()==21))
			      ...
			      else
				{
			      ...
			      // Sangyong's addition
			      parton.elastic_only(0); // no limits on scatterings
			      ...
			      parton.plist_pos(plist->size()); // its current position in the plist 
			      // Sangyong's addition
...
bool MARTINI::init(int argc, char** argv) 
{
  ...
  runningCoupling = settings.flag("General:RunningCoupling");
  cout << "runningCoupling = " << runningCoupling << endl;
...
*/
  ...
  if(tauEtaCoordinates == 1 || hydroWhichHydro == 6 ){
    hydroTauMax = hydroTau0 + hydroDtau*(int)((double)lattice->size()/( (2.*hydroXmax/hydroDx+1.)*(2.*hydroXmax/hydroDx+1.)*2.*(hydroZmax/hydroDz) ) ) ;
  }
  else{
...
vector<Parton>* MARTINI::next()
{
  ...
  cnt++;
  if(cnt % 10 == 0) printf("In MARTINI::next, cnt = %d\n", cnt);
  ...
  if (fullEvent==1)
		  ...
		  if ( random->genrand64_real1() < totalHQXSec/inelasticXSec )
		      ...
		      for (int ip = 0; ip < pythia.event.size(); ++ip) 
			{
			  if (pythia.event[ip].status()>0 && (abs(pythia.event[ip].id())==4 || abs(pythia.event[ip].id()) == 5) )
				...
				parton.p(0.,0.,0.);
				parton.pOld(0.,0.,0.);
				Vec4 pinitial(pythia.event[ip].mSel(),0.,0.,0.);
      ...
      for (int i = 0; i < pythia.event.size(); ++i) 
	{
	  if (pythia.event[i].isFinal() && (abs(pythia.event[i].id()) == 4 || abs(pythia.event[i].id()) == 5) )
		...
		Vec4 pinitial(pythia.event[i].mSel(), 0.,0.,0.);
	...
	if(examineHQ == 2){
	  if(id == 4 || id == 5){ //Only the quarks, not the anti-quarks!
	    ...
	    if(fixedTemperature == 0){
	      ...
	      if(tauEtaCoordinates==1){
		antihydroInfo = hydroSetup->getHydroValuesTauEta(antix, antiy, antiz, t, hydroXmax, hydroZmax, hydroTauMax, hydroTau0, 
								 hydroDx, hydroDz, hydroDtau, hydroWhichHydro, fixedDistribution, lattice, trackHistory);
	      }
	      else{
...
int MARTINI::hasDaughter(vector<Parton> *plist, int i)
{
 int daughter;
...
int MARTINI::hasMother(vector<Parton> *plist, int i)
{
 int mother;
 double x, y, z, rperp, kx, ky, kz, kox, koy, koz, ko;
 double kcrx, kcry, kcrz, kperp;
