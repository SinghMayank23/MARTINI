...
int MARTINI::evolve(vector<Parton>  **plist, int counter, int it)
{
  ...
  for ( int i=0; i<imax; i++)                                   // loop over all partons
      ...
      if (fixedTemperature == 0)
	{
	  ...
	  ////Get rid of these cout statements!:
	  //cout << "|\\vec{p}| = " << p << ", tau = " << tau << endl;
	  ...
	  if (hydroWhichHydro == 3 || hydroWhichHydro == 4 || hydroWhichHydro == 1 || hydroWhichHydro == 5 || hydroWhichHydro == 6) 
	    {
	  ...
	  if (QGPfrac == 0. && (hydroWhichHydro == 3 || hydroWhichHydro == 1 || hydroWhichHydro == 4 || hydroWhichHydro == 5 || hydroWhichHydro == 6) )
	    {
	      plist[0]->at(i).frozen(1);
	      plist[0]->at(i).tFinal(t);
	      lengthTraveled = sqrt((x-plist[0]->at(i).xini())*(x-plist[0]->at(i).xini())
	  ...
	  else if (QGPfrac < 1. && (hydroWhichHydro == 3 || hydroWhichHydro == 1 || hydroWhichHydro == 4 || hydroWhichHydro == 5 || hydroWhichHydro == 6))
	  ...
	  ////Get rid of these cout statements!:
	  //cout << pRest << ">" << pCut << " with the original p = " << p << endl;
      ...
      if ( abs(id) > 0 && abs(id) < 4 ) // if parton is a quark (u, d, or s), let it evolve like a quark
	{
	  ...
	  if (doRadiative == 1 && doElastic == 1 ) //radiative + elastic 
	      ...
	      if ( random->genrand64_real1() < totalQuarkProb ) // check if something happens with the quark
		{
		  ...
		  ////Get rid of these cout statements!:
		  //cout << "Quark with momentum " << p << " evolved..." << endl;
...
bool MARTINI::init(int argc, char** argv) 
{
  ...
  if(evolution == 1 && fixedTemperature == 0){
    ...
    //  hydroInfo = hydroSetup->getHydroValuesTauEta(0., 0., 0., hydroTau0, hydroXmax, hydroZmax, hydroTauMax, hydroTau0, 
    //						   hydroDx, hydroDz, hydroDtau, hydroWhichHydro, fixedDistribution, lattice, trackHistory);
    //}
    //else{
    ...
    for(int inum=0; inum<numpoints; inum++){
      ...
      double hydroTau = sqrt((10.-hydroTau0)*(10.-hydroTau0)-hydroZ*hydroZ );
	...
	cout << (double)inum*(zrange/(double)numpoints) << " " << hydroInfo.T << " " << hydroInfo.vx
	     << " " << hydroInfo.vz << " " << 0.5*log((1.+hydroInfo.vz)/(1.-hydroInfo.vz)) << endl;
      }
    }
  }
