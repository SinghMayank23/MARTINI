...
class Parton
{
 private:
  ...
  int itsPlistPosition; // current position in the plist
  ...
  int ElasticOnly; // 1 means elastic only. 0 means rad + el.
...
//Sangyong's additions start
  // plist position
  int plist_pos() const {return itsPlistPosition; }
  void plist_pos(int value) { itsPlistPosition=value; }
  ...
  // elastic only
  int elastic_only() const {return ElasticOnly; }
  void elastic_only(int value) { ElasticOnly=value; }
//Sangyong's additions end
};
