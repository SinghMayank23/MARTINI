...
int main(int argc, char* argv[]) 
{
  ...
  martini.readFile("setup_main9.dat");
  ...
  n_string = "PbPb_withneutral_"+n_string;
  ...
  while(event_counter<numJets)      // The main loop
	...
	//First of all, if this dijet triggers ATLAS, output ALL charged, final hadrons for 
	//later tweaks of the analysis:
	for(int ip=0; ip<fullEventSize; ip++){
	  if(fullEvent[ip].isCharged() && fullEvent[ip].isHadron() && fullEvent[ip].isFinal() ){
	    foutEvents << fullEvent[ip].m() << " " << fullEvent[ip].pT()*cos(fullEvent[ip].phi() ) << " "
