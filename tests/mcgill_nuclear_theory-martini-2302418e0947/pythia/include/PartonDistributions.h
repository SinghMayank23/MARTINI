...
public:
  ...
  double xf(int id, double x, double Q2, bool isNeutron=0);

  // Read out valence and sea part of parton densities.
  double xfVal(int id, double x, double Q2, bool isNeutron=0);
  double xfSea(int id, double x, double Q2, bool isNeutron=0);
...
public:
  ...
  // Set parameters for nuclear effects (bps).
  void setNuclearEffects(int value)
    { nuclearEffects = value; }
...
private:
  ...
  // Option for nuclear effects (bps).
  int nuclearEffects;
  // Atomic number (bps).
  double a;
