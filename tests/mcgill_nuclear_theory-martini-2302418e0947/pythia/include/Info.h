...
public:
  ...
  // Integrated inelastic cross section from MultipleInteractions class. *added by schenke for MARTINI - 2009/04/16
  double sigmaInt()       const {return sigmaIntMI;}
...
private:
  ...
  long   nTry, nSel, nAcc;
  double sigGen, sigErr, sigmaIntMI;
  ...
  // Set info on total cross section.
  void setSigmaIntMI( double sigmaIntMIIn ) {sigmaIntMI = sigmaIntMIIn;}
