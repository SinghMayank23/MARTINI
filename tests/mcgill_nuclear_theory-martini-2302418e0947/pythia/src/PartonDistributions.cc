...
double PDF::xf(int id, double x, double Q2, bool isNeutron) { 

  // switch for neutron pdfs (switching xu<->xd) added by Bjoern Schenke 09/08/09
  ...
  if (abs(idBeam) > 100) { 
    ...
    if (idNow == 1 && isNeutron==0 ) return max(0., xd);
    else if (idNow == 1 && isNeutron==1 ) return max(0., xu);
    //if (idNow == 1) return max(0.,79./197.*xd+(118./197.*xu));
    
    if (idNow == -1 && isNeutron==0 ) return max(0., xdbar);
    else if (idNow == -1 && isNeutron==1 ) return max(0., xubar);

    if (idNow == 2 && isNeutron==0 ) return max(0., xu);
    else if (idNow == 2 && isNeutron==1 ) return max(0., xd);
    //if (idNow == 2) return max(0.,79./197.*xu+(118./197.*xd));
   
    if (idNow == -2 && isNeutron==0 ) return max(0., xubar);
    else if (idNow == -2 && isNeutron==1 ) return max(0., xdbar);
...
double PDF::xfVal(int id, double x, double Q2, bool isNeutron) { 
  ...
  if (abs(idBeam) > 100) { 
    int idNow = (idBeam > 0) ? id : -id;
    if (idNow == 1 && isNeutron==0 ) return max(0., xdVal);
    else if (idNow == 1 && isNeutron==1 ) return max(0., xuVal);
    //if (idNow == 1) return max(0.,79./197.*xdVal+(118./197.*xuVal));
  
    if (idNow == 2 && isNeutron==0 ) return max(0., xuVal);
    else if (idNow == 2 && isNeutron==1 ) return max(0., xdVal);
    //if (idNow == 2) return max(0.,79./197.*xuVal+(118./197.*xdVal));
    return 0.;
...
double PDF::xfSea(int id, double x, double Q2, bool isNeutron) { 
  ...
  if (abs(idBeam) > 100) { 
    ...
    if (idNow == 1 && isNeutron==0 ) return max(0., xdSea);
    else if (idNow == 1 && isNeutron==1 ) return max(0., xuSea);

    if (idNow == -1 && isNeutron==0 ) return max(0., xdbar);
    else if (idNow == -1 && isNeutron==1 ) return max(0., xubar);
    
    if (idNow == 2 && isNeutron==0 ) return max(0., xuSea);
    else if (idNow == 2 && isNeutron==1 ) return max(0., xdSea);
...
void LHAPDF::xfUpdate(int , double x, double Q2) {
  ...
  if (nuclearEffects == 1)
    {
      //cout << "using pythia evolvePDFaM with A=" << a << endl;
      LHAPDFInterface::evolvePDFaM( nSet, x, Q, a, xfArray);
    }
  else
    {
