...
Pythia::Pythia(string xmlDir) { 
  ...
  // initial value for jetpTmin (used by MARTINI)
  jetpTmin = 0.2;
...
bool Pythia::initInternal() {
  ...
  if ( doPartonLevel && !partonLevel.init( jetpTmin, &info, &beamA, &beamB, 
...
bool Pythia::next(bool n1, bool n2) {
...
PDF* Pythia::getPDFPtr(int idIn, int sequence) {
  ...
  if (abs(idIn) == 2212 && sequence == 1) {
    ...
    // Use sets from LHAPDF.
      ...
      // Include nuclear effects (bps).
      //cout << " see if pythia uses nuclear effects:" << settings.mode("PDF:nuclearEffects") << endl;
      tempPDFPtr->setNuclearEffects( settings.mode("PDF:nuclearEffects") );
      tempPDFPtr->setA( settings.parm("PDF:atomicNumber") );
    }
  }
