...
void TimeShower::init( BeamParticle* beamAPtrIn, 
  ...
  // tauHydro used in pTmin for shower when MARTINI is used - added by Bjoern Schenke 09/28/2009
  doMedium           = Settings::flag("TimeShower:doMedium");
  tauHydro           = Settings::parm("TimeShower:tauHydro")/0.197327053;
}
...
void TimeShower::pT2nextQCD(double pT2begDip, double pT2sel, 
  ...
  // Lower cut for evolution. Return if no evolution range. -  modified by Bjoern Schenke 09/28/2009
  double mediumCut;
  double tempEndDip = max( pT2sel, pT2colCut );
  double pT2endDip = tempEndDip;
  ...
  do { 
    ...
    // Abort evolution if below cutoff scale, or below another branching.
      ...
      if (doMedium)
	mediumCut=dip.z*(1.-dip.z)*event[dip.iRadiator].e()/tauHydro;
      else
	mediumCut=0;
      pT2endDip = max( tempEndDip, mediumCut );   
      //cout << "mediumCut=" << mediumCut << endl;
      //cout << "z=" << dip.z << endl;
      //cout << "tauHydro=" << tauHydro << endl;
      //cout << "old minimal pT2=" << tempEndDip << endl;
      //cout << "new minimal pT2=" << pT2endDip << endl;
      //cout << "pT2begDip=" << pT2begDip << endl << endl;
      //cout << "pT=" << event[dip.iRadiator].pT() << endl;
      if ( dip.pT2 < pT2endDip) { 
	//cout << " actually exiting..." << endl;
	//cout << "old minimal pT2=" << tempEndDip << endl;
	//cout << "new minimal pT2=" << pT2endDip << endl;
	dip.pT2 = 0.; return; }
...
void TimeShower::pT2nextQED(double pT2begDip, double pT2sel, 
  ...
  double tempEndDip = max( pT2sel, pT2chgCut );
  double pT2endDip = tempEndDip;
  if (pT2begDip < pT2endDip) return;   
