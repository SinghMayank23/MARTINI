	  ...
	  if ( martini.returnFullEvent() == 0 ) 
	    {
	      ...
	      cellJet.analyze2( martini.pythia.event, 10., 0.7, 2., 2.);
	      ...
	      for (int i=0; i<totalNNs; i++)
		{
		  ...
		  cellJet.analyze2( martini.pythia.event, 10., 0.38, 2., 2.);
		  // jets will be automatically sorted with decreasing eT
		  if (cellJet.size()>=1)
		    {
		  ...
		  double dalitzX[3];
		  int relevantJets=0;
		  cellJet.analyze2( fullEvent, 10., 0.4, 2., 10.); //second to last parameter is pTmin for each particle to be accepted
		  //as part of the jet
		  // jets will be automatically sorted with decreasing eT
