...
double Elastic::totalRate(double p, double T, double alpha_s, int Nf, int process, double multElasticRate)
{
  ...
  if (multElasticRate>1.)
    rate*=multElasticRate;
  return rate;
}
	  ...
	  do
	    {
	      ...
	      //if (i>200)
	      // {
	      //	 cout << "(omega>0,process 3 or 4) with p=" << u << ", we" << " are having " << i << " rejections." << endl;
	      // }
					  ...
					  Random *random, Import *import, int process)
{
  ...
  if ( omega < 10. && omega >-3 )
      ...
      do
	{
      ...
      if (i>2000)
	{
          cout << " with omega=" << omega << ", we" << " had " << i << " rejections." << endl;
	}
    }
...
Vec4 Elastic::getNewMomentum(Vec4 vecpRest, double omega, double q, Random *random)
{
  ...
  if (q<sqrt(omega*omega)) cout << "WARNING: q<omega=" << q << "<" << omega << endl;
