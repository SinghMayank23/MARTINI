...
// Initialization in the CM frame by MARTINI - added by Chanwook Park 12/12/2014
...
bool Pythia::init() {
  ...
  if (frameType < 4 ) {
    doLHA     = false;
    boostType = frameType;
  //added by Chanwook Park not to be set twice.
    if(!isCMInit) {
    idA       = mode("Beams:idA");
    idB       = mode("Beams:idB");
    eCM       = parm("Beams:eCM");
  ...
  if ( doPartonLevel && doProcessLevel && !partonLevel.init( jetpTmin, jetpTmax, &info, settings,
  ...
  if ( !doProcessLevel || !doPartonLevel) partonLevel.init( jetpTmin, jetpTmax, &info, settings,
  ...
  if ( doMerging && !trialPartonLevel.init( jetpTmin, jetpTmax, &info, settings, &particleData,
      &rndm, &beamA, &beamB, &beamPomA, &beamPomB, couplingsPtr,
      &partonSystems, &sigmaTot, timesDecPtr, timesPtr, spacePtr, &rHadrons,
  ...
  jetXSec = partonLevel.jetCrossSection();
  totaljetXSec = partonLevel.totaljetCrossSection();
  totalXSec = partonLevel.totalCrossSection();
  elasticXSec = partonLevel.elasticCrossSection();
...
bool Pythia::next(int n1, int n2) {

  // set beam to be a neutron if demanded - added by Bjoern Schenke 09/08/09
  beamA.AbsidNow(n1);
  beamB.AbsidNow(n2);
...
PDF* Pythia::getPDFPtr(int idIn, int sequence, string beam) {
  ...
  if (abs(idIn) == 2212 || abs(idIn) == 2112) {
    ...
    if (pSetInt == 0)
    {
      ...
      // set parameters for nuclear pdf (cshen)
      int nuclearEffectFlag = settings.mode("PDF:nuclearEffects");

      int atomic_number;
      if(beam == "B")
        atomic_number = (int)settings.parm("PDF:targ_atomicNumber");
      else
        atomic_number = (int)settings.parm("PDF:proj_atomicNumber");

      tempPDFPtr->setAtomicNumber(atomic_number);

      if(atomic_number == 1) 
        nuclearEffectFlag = 0;
      
      tempPDFPtr->setNuclearEffects(nuclearEffectFlag);
      if(nuclearEffectFlag == 1)
      {
        string tempString = settings.word("PDF:nuclearPDF");
        tempPDFPtr->setNuclearPDF(tempString);
      }
    }
