...
namespace LHAPDF5Interface {
  ...
  // Evaluate x f_i(x, Q) including nuclear corrections. a is the atomic number      
...
public:
  ...
  void setNuclearEffects(int value) {nuclearEffects = value;};
  void setNuclearPDF(string value){
     cout << "LHAPDF5: using nuclear PDF: " << value << endl;
     cout << "LHAPDF5: nucleus = " << atomicNumber << endl;
     LHAPDF5Interface::setPDFparm( value );
  };
  void setAtomicNumber(int value) {atomicNumber = value;};
...
private:
  ...
  // parameters for nuclear pdf (cshen)
  int nuclearEffects;
  string nuclearPDFname;
  int atomicNumber;
...
void LHAPDF5::xfUpdate(int, double x, double Q2) {
  ...
  // Else use default LHAPDF5 call.
    ...
    if(nuclearEffects == 1)
    {
       //cout << "using pythia evolvePDFaM with A=" << atomicNumber << endl;
       LHAPDF5Interface::evolvePDFaM( nSet, x, Q, (double)atomicNumber, xfArray);
    }
    else
    {
