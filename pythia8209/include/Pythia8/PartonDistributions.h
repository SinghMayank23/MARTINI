...
public:
  ...
  virtual void setNuclearEffects(int) {};
  virtual void setNuclearPDF(string) {};
  virtual void setAtomicNumber(int) {};

  // Read out parton density
  virtual double xf(int id, double x, double Q2, int idBeamAbsNow);

  // Read out valence and sea part of parton densities.
  virtual double xfVal(int id, double x, double Q2, int idBeamAbsNow);
  virtual double xfSea(int id, double x, double Q2, int idBeamAbsNow);
...
public:
  ...
  // set parameters for nuclear pdf (cshen)
  void setNuclearEffects(int value) {
    if (pdfPtr) pdfPtr->setNuclearEffects(value);}
  void setNuclearPDF(string value) {
    if (pdfPtr) pdfPtr->setNuclearPDF(value); }
  void setAtomicNumber(int value) {
    if (pdfPtr) pdfPtr->setAtomicNumber(value);}

  // Read out parton density
  double xf(int id, double x, double Q2, int idBeamAbsNow) {
    if (pdfPtr) return pdfPtr->xf(id, x, Q2, idBeamAbsNow); else return 0;}

  // Read out valence and sea part of parton densities.
  double xfVal(int id, double x, double Q2, int idBeamAbsNow) {
    if (pdfPtr) return pdfPtr->xfVal(id, x, Q2, idBeamAbsNow); else return 0;}
  double xfSea(int id, double x, double Q2, int idBeamAbsNow) {
    if (pdfPtr) return pdfPtr->xfSea(id, x, Q2, idBeamAbsNow); else return 0;}
