  ...
  if(examineHQ == 1){
    ...
    if(T > T_C_HQ){
      ...
      if(HQ_rate_type == 0)
      {
        ...
        eta = 2.*M_PI*T*T/(TwoPiTD_HQ*1.25);
        kappa_L = kappa_T;
...
//          temp_2piTD = 20.30405*T + 0.35256;
	...
	} else
	{
        ...
        eta = 2.*M_PI*T*T/(temp_2piTD*1.25);
        kappa_L = kappa_T;
        ...
        if (fixedTemperature == 0)
    {
      ...
      if(evolution == 1){
        if(tau<hydroTau0 && tau > prehydroTau0 && prehydroevolution == 1)
        {
          ...
          T = prehydroInfo.T;
          umu[0] = prehydroInfo.utau; 
          umu[1] = prehydroInfo.ux; 
          umu[2] = prehydroInfo.uy; 
          umu[3] = 0.;//Boost invariant prehydro
          LorentzBooster(umu, pmu, pmuRest);                      // Boost into the fluid's rest frame
          dtflow = dt*pmuRest[0]/pmu[0];                          // dtfm in the fluid's rest frame
	  if (T > T_C_HQ){
...
//                 hydroDx, hydroDz, hydroDtau, hydroWhichHydro, fixedDistribution, lattice, trackHistory);
	...
	else
	{
      ...
      if(tau<hydroTau0 && moveBeforeTau0 == 2 && (prehydroevolution == 0 || T < T_C_HQ))
      {
        ...
        if(plist[0]->at(antiI).frozen() == 0)
	{
      ...
      if ((tau<hydroTau0 && prehydroevolution == 0) || (tau>hydroTauMax && evolution == 1) || tau < prehydroTau0) continue;       // do not evolve if tau<tau0 or tau>tauMax
...
//          temp_2piTD = 20.30405*T + 0.35256;
    ...
    plist[0]->at(i).p(pmuNew[1], pmuNew[2], pmuNew[3]);
        ...
        else if (T < T_C_HQ && (prehydroevolution == 0 || tau > hydroTau0))
     {
    ...
    if(plist[0]->at(antiI).frozen() == 0){
      ...
      int bound = 0;//isBound(id1, id2, E_CM);
    ...
    if(numberOfNeighbors == 0){
      ...
      if (do_coalesence)
      {
          int does_it_coalesce = 0;//check_coalescence(id1, p1mu, M1, x, y, z, t);
...
int MARTINI::check_coalescence(int id1, double *pmu, double M1, double x, double y, double z, double t){
    ...
    if (random < probability_to_coalesce) decision = 2; //Coalescening
    if (random < probability_to_coalesce && random < 0.4*probability_to_coalesce) decision = 1;//Coalescening to meson
...
bool MARTINI::hadronize_by_coalescence(int id1, double *pHQ, double M, double x, double y, double z, double t){
   ...
   while(!coalesence_complete && counter < 100000)
   {
     ...
     prob *= Coal_normal*pow(2.*sqrt(M_PI)*sigma, 3.)/36.;
