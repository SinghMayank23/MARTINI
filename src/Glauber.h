...
class Glauber
{
 private:
  ...
  int Projectile_A, Target_A;
  
 public:
  int get_Projetile_A() {return Projectile_A;} 
  int get_Target_A() {return Target_A;} 
  int get_Projetile_Z() {return(LexusData.Projectile.Z);}
  int get_Target_Z() {return(LexusData.Target.Z);}

  int IsFile(char *file_name);
  void FindNucleusData(Nucleus *nucleus, char *target, char *file_name);
  void lookup_nucleus_info(Nucleus *nucleus, string nucleus_name);
  void PrintLexusData();
  void PrintNucleusData(Nucleus *nucleus);
  void PrintCollisionSystem(Data *nucleus);
