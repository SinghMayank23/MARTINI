...
int main(int argc, char* argv[]) 
{
  ...
  MARTINI martini;
  Event fullEvent;
  HydroSetup* hydroSetup = new HydroSetup();
  hydroSetup->readPreHydroData(0.0166666666666667, 0.4, 0.0166666666666667,
                                  17.0, 0.0664062, "epsilon-u-Hydro-t0."); 
  hydroSetup->readHydroData(0.4, 20., 0.1,
                                  17., 10., 0.2656248, 0.3125, 
                                  1, 1, 1,
                                  8, 0.165, 1,
                                  0, 0,
                                  "evolution_xyeta.dat", "a.a", "a.a");
	  ...
	  for(int i=0; i<mt; i++)
	    {
	      ...
	      double time = ((double)i+1.)*dtfm;
              ...
              for (int ie = 0; ie < eventsize; ie++){
                ...
                double x = plist[0]->at(ie).x();
                double y = plist[0]->at(ie).y();
	        Vec4 itsP = plist[0]->at(ie).p();
	        double px = itsP.px();
	        double py = itsP.py();
	        double pz = itsP.pz();
		double tau = sqrt(time*time - z*z);
		double sprap = 0.5*log((time+z)/(time-z));
		double pt = sqrt(px*px + py*py + pz*pz);
		double Ener = sqrt(plist[0]->at(ie).mass()*plist[0]->at(ie).mass() + pt*pt);
		double momrap = 0.5*log((Ener + pz)/(Ener - pz));
		double psurap = 0.5*log((pt + pz)/(pt - pz));
		HydroInfo hydroInfo;// antihydroInfo;
                PreHydroInfo prehydroInfo;// antihydroInfo;

		double temperature = 0.;
                if (tau<0.4 && tau > 0.0166666666666667)
                {   
                  prehydroInfo = hydroSetup->getPreHydroValues(x, y, z, time);
		  temperature = prehydroInfo.T;
		} else if (tau >= 0.4) {
                  hydroInfo = hydroSetup->getHydroValues(x, y, z, time);
		  temperature = hydroInfo.T;
		}

		if (abs(plist[0]->at(ie).id())==4 && plist[0]->at(ie).frozen()==0){
	        cout << ie << " " << time << " " << tau << " " << sprap << " " << momrap << " " << psurap << " " << x << " " << y << " " << z << " " << px << " " << py << " " << pz << " " << temperature << endl;
		}
	      }
